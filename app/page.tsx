        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {portfolioData.projects.map((project, i) => (
            <motion.article
              ...
              className="glass grid-glow relative rounded-3xl p-6"
            >
              <div className="mb-4 flex items-center justify-between gap-3">
                <h3 className="text-xl font-semibold text-white">{project.title}</h3>
                <span className="rounded-full bg-white/10 px-3 py-1 text-xs text-white/70">{project.status}</span>
              </div>
              ...
              <div className="mt-6 flex gap-3">
                {project.github ? (
                  ...
                  <span className="rounded-full bg-white/10 px-4 py-2 text-sm text-white/70">Private Project</span>
                )}
              </div>
            </motion.article>
          ))}
      ...
      <motion.section id="contact" className="mx-auto mt-20 max-w-4xl" {...fadeUp}>
        <div className="glass rounded-3xl p-8 text-center shadow-glow">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
            <a href={`mailto:${portfolioData.personal.email}`} className="rounded-full bg-white px-6 py-3 font-medium text-slate-900">
              Email Me
            </a>
            <a href={portfolioData.personal.linkedin} target="_blank" rel="noreferrer" className="glass rounded-full px-6 py-3 font-medium text-white">
              LinkedIn
            </a>
